...
import { EditTapAuthorisationLocationPage } from './test.page'
import { testNotAuthorisedPage } from '../../../../../../integration_tests/steps/testNotAuthorisedPage'
import { stubGetAddress, stubSearchAddresses } from '../../../../../../integration_tests/mockApis/osPlacesApi'

test.describe('/temporary-absence-authorisations/edit/location unauthorised', () => {
  test('should show unauthorised error', async ({ page }) => {
    await testNotAuthorisedPage(page, `/${uuidV4()}/temporary-absence-authorisations/edit/location`)
  })
})

test.describe('/temporary-absence-authorisations/edit/location', () => {
  ...
  const startJourney = async (page: Page, journeyId: string) => {
    await page.goto(`/${journeyId}/temporary-absence-authorisations/start-edit/${authorisationId}/location`)
  }

  test('should search and select a UK address and proceed to change-confirmation page', async ({ page }) => {
    const journeyId = uuidV4()
    await startJourney(page, journeyId)

    // verify page content
    const testPage = await new EditTapAuthorisationLocationPage(page).verifyContent()
  ...
  test('should search and select a location and proceed to change-confirmation page', async ({ page }) => {
    const journeyId = uuidV4()
    await startJourney(page, journeyId)
  ...
  test('should enter an address and proceed to change-confirmation page', async ({ page }) => {
    const journeyId = uuidV4()
    await startJourney(page, journeyId)
  ...
  test('should enter an area and proceed to change-confirmation page', async ({ page }) => {
    const journeyId = uuidV4()
    await startJourney(page, journeyId)
