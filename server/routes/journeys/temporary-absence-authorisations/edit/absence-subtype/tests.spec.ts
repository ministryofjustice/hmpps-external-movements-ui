...
import { EditAbsenceSubTypePage } from './test.page'
import { testNotAuthorisedPage } from '../../../../../../integration_tests/steps/testNotAuthorisedPage'

test.describe('/temporary-absence-authorisations/edit/absence-subtype unauthorised', () => {
  test('should show unauthorised error', async ({ page }) => {
    await testNotAuthorisedPage(page, `/${uuidV4()}/temporary-absence-authorisations/edit/absence-subtype`)
  })
})

test.describe('/temporary-absence-authorisations/edit/absence-subtype', () => {
  ...
  test.beforeAll(async () => {
    await Promise.all([
      ...
      stubGetAbsenceCategory('ABSENCE_TYPE', 'RR'),
    ])
  })
  ...
  const startJourney = async (page: Page, journeyId: string) => {
    await page.goto(`/${journeyId}/temporary-absence-authorisations/start-edit/${authorisationId}/absence-subtype`)
  }

  test('should confirm and save absence subtype', async ({ page }) => {
    const journeyId = uuidV4()
    await startJourney(page, journeyId)

    // verify page content
    const testPage = await new EditAbsenceSubTypePage(page).verifyContent()

    await expect(testPage.crlRadio()).toBeVisible()
    await expect(testPage.splRadio()).toBeVisible()
    await expect(testPage.rdrRadio()).toBeVisible()
  ...
  test('should select absence subtype and continue to next page', async ({ page }) => {
    const journeyId = uuidV4()
    await startJourney(page, journeyId)
    ...
    await page.goBack()
    await page.reload()
    await expect(testPage.splRadio()).toBeChecked()
  })
})
