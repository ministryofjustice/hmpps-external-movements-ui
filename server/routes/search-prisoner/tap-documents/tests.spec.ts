...
test.describe('/search-prisoner/tap-documents', () => {
  ...
  test('should try all cases', async ({ page }) => {
    await page.goto('/search-prisoner/tap-documents')

    // verify page content
    const testPage = await new TapDocumentSearchPrisonerPage(page).verifyContent()
    ...
    await expect(testPage.link('Create a document for Name One')).toHaveAttribute(
      'href',
      /\/tap-documents\/start\/A1234EA/,
    )

    await testPage.verifyTableRow(2, ['Another Two', 'A2345EA', 'Loc-2'])
    await expect(testPage.link('Create a document for Another Two')).toHaveAttribute(
      'href',
      /\/tap-documents\/start\/A2345EA/,
    )
  })
})
